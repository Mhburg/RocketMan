...
namespace RocketMan
{
        ...
        where TInterval : new ()
    {
        ...
        public CacheableBase(TType t, Func<TTime> now, TInterval interval, Func<TType> update, TTime lastUpdateTime = default)
        {
            _backingFiled = t;
    ...
    {
        public CacheableTime(TType t, Func<DateTime> now, TimeSpan updateInterval, Func<TType> update, DateTime lastUpdateTime = default)
        ...
        {
            if (!cache.ShouldUpdate(out DateTime now))
            {
        ...
        {
            now = this.Now();
            return !(this.LastUpdateTime + this.UpdateInterval > now);
        }
    }
    ...
    public class CacheableTick<TType> : CacheableBase<TType, int, int>
    {
        ...
        {
            if (!cache.ShouldUpdate(out int now))
            {
                return cache._backingFiled;
            }
            else
